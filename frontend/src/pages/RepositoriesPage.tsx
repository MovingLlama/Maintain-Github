...
type MobileTab = 'repos' | 'browse' | 'view'
...
export function RepositoriesPage() {
  ...
  const selectRepo = useCallback((repo: Repository) => {
    ...
    setShowIssuesTab(false)
    if (isMobile) setMobileTab('browse')
  }, [isMobile, setSelectedRepo])
  ...
  const browsePanel = selectedRepo ? (
    <div className="flex flex-col h-full">
      {/* Tab Switcher */}
  ...
  const viewPanel = selectedRepo && selectedFile && fileContent !== null ? (
    <div className="h-full overflow-y-auto p-4">
      <div className="flex items-center justify-between mb-3">
        <span className="text-sm text-gray-400 font-mono break-all">{selectedFile}</span>
      </div>
      <SyntaxHighlighter
  ...
  // ---- Modals (shared) ----
  const githubModal = showGithub && (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setShowGithub(false)}>
      <div className="bg-gray-900 border border-gray-700 rounded-xl w-full max-w-lg mx-4 p-6 max-h-[80vh] flex flex-col" onClick={e => e.stopPropagation()}>
  ...
  const agentModal = showAgentModal && (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setShowAgentModal(null)}>
      <div className="bg-gray-900 border border-gray-700 rounded-xl w-full max-w-md mx-4 p-6 max-h-[80vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
  ...
  const pushModal = showPushModal && (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setShowPushModal(false)}>
      <div className="bg-gray-900 border border-gray-700 rounded-xl w-full max-w-md mx-4 p-6" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
      ...
      <div className="h-full flex flex-col bg-gray-950">
        ...
        <div className="flex border-b border-gray-800 bg-gray-900 shrink-0">
          {([
            ...
            { key: 'browse' as MobileTab, label: selectedRepo ? 'Browse' : 'Browse' },
            { key: 'view' as MobileTab, label: selectedFile ? 'View' : 'View' },
          ]).map(tab => (
            <button
              key={tab.key}
            ...
            >
              {tab.label}
              {tab.key === 'browse' && selectedRepo && (
                <span className="ml-1 text-sky-400">•</span>
              )}
              {tab.key === 'view' && selectedFile && (
                <span className="ml-1 text-sky-400">•</span>
              )}
            </button>
          ))}
